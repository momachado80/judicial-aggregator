...
@router.post("", response_model=List[ProcessoResponse])
async def buscar_processos(request: BuscarProcessosRequest):
    ...
    for tribunal in request.tribunais:
        ...
        if request.tipos_processo:
            tipo_clauses = [{"match": {"classe.nome": tipo}} for tipo in request.tipos_processo]
            must_clauses.append({"bool": {"should": tipo_clauses, "minimum_should_match": 1}})
        ...
        if request.comarcas:
            ...
            for comarca in request.comarcas:
                ...
                if pattern:
                    comarca_patterns.append({"wildcard": {"numeroProcesso": pattern}})
            if comarca_patterns:
                must_clauses.append({"bool": {"should": comarca_patterns, "minimum_should_match": 1}})
        ...
        if request.ano_ajuizamento:
            must_clauses.append({"match": {"dataAjuizamento": str(request.ano_ajuizamento)}})
...
async def listar_comarcas(tribunal: str):
    ...
    return {
        "tribunal": tribunal,
        "total": len(comarcas),
        "comarcas": [{"nome": nome, "codigo": codigo} for nome, codigo in sorted(comarcas.items())]
    }
