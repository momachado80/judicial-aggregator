...
def baixar_historico(anos=2, cadernos=["11", "12", "13", "14"], continuar_de=None):
    """
    Baixa PDFs históricos do DJE

    Args:
        anos: Quantos anos para trás baixar (2 ou 4)
        cadernos: Lista de cadernos a baixar (padrão: todos - 11, 12, 13, 14)
    ...
    print(f"📚 Cadernos: {', '.join(cadernos)} (Capital + Interior)")
    print(f"⏱️  Isso pode levar HORAS (há milhares de PDFs)")
    print("="*80)

    # Gerar lista de dias úteis
    dias_uteis = gerar_dias_uteis(data_inicio, data_fim)
    print(f"\n📊 {len(dias_uteis)} dias úteis × {len(cadernos)} cadernos = {len(dias_uteis) * len(cadernos)} PDFs potenciais")
    ...
    total_tentativas = len(dias_uteis) * len(cadernos)
    tentativa_atual = 0
    ...
    for data in dias_uteis:
        for caderno in cadernos:
            ...
            if filename in pdfs_existentes:
                print(f"[{tentativa_atual}/{total_tentativas}] ⏩ {data} Cad{caderno} - já existe")
                pulados += 1
                continue

            try:
                print(f"\n[{tentativa_atual}/{total_tentativas}] 📥 Baixando {data} - Caderno {caderno}...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cadernos", type=str, nargs='+', default=["11", "12", "13", "14"],
                       help="Cadernos a baixar (padrão: todos - 11 12 13 14)")
